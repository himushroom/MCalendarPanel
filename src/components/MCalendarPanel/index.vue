			...
			>
				<div
					:class="[
						'line',
						{
							'now-day': isToday === item.isToday,
							select: isSelect === item.isToday,
							selectClassName: isSelect === item.isToday,
						},
					]"
					...
					<template v-else>
						...
						<template v-else>
							...
							<div
								...
								:style="festivalStyle"
							>
								{{ item.lunar.festival[0] }}
							</div>
							<div v-else class="extra">
								{{
									item.dateNum
										? `${currentLunar(item.lunar)}`
										: ""
								}}
							</div>
						</template>
			...
			for (let i = 0; i < fullNumber; i++) {
				...
				if (i >= startWeek && i < arrLen) {
					...
					obj = {
						...
						dateNum: prevDay,
						lunar: getSolarLunar(
							+new Date(prevYear, prevMonth - 1, prevDay)
						),
						notNowMonth: "prev"
					};
				} else {
					...
					obj = {
						...
						dateNum: nextDay,
						lunar: getSolarLunar(
							+new Date(nextYear, nextMonth - 1, nextDay)
						),
						notNowMonth: "next"
					};
				}
